...
public class ThePagerWSearch {
	 ...
	 public String toString(){
	  ...
	  if (currentPage > 1) {
	   linkString.append(
	    String.format("[<a href='%s?cp=1?option=%s&option=%s&option=%s&option=%s'>처음</a>]",linkUrl,option[0],option[1],option[2],option[3]));
	   linkString.append("&nbsp;");
	   linkString.append("&nbsp;");
	   linkString.append(String.format(
	    "[<a href='%s?cp=%d?option=%s&option=%s&option=%s&option=%s'>이전</a>]", linkUrl, currentPage - 1,option[0],option[1],option[2],option[3]));
	   linkString.append("&nbsp;");
	  }
	  ...
	  for (int i = start; i < end; i++) {
	   ...
	   if(i == currentPage) { // 현재 페이지가 i 라면 번호 형성 [ i ] 처럼 현재 해당되는 위치에 괄호 표시
	    ...
	    linkString.append(String.format( // 그렇지 않으면 i에 해당하는 페이지에 해당하는 번호 형성 ( !!! [ ] 괄호 없음 )
	     "<a href='%s?cp=%d?option=%s&option=%s&option=%s&option=%s'>%d</a>", linkUrl, i, i,option[0],option[1],option[2],option[3]));
	   }
	   linkString.append("&nbsp;");
	  }
	  ...
	  if (currentPage < pageCount) { // 현재페이지가 총 페이지보다 작다면
	   linkString.append("&nbsp;");
	   linkString.append(String.format(
	    "[<a href='%s?cp=%d?option=%s&option=%s&option=%s&option=%s'>다음</a>]",linkUrl, currentPage + 1,option[0],option[1],option[2],option[3])); // 다음으로 가는 페이지 형성
	   linkString.append("&nbsp;");
	   linkString.append("&nbsp;");
	   linkString.append(String.format(
	    "[<a href='%s?cp=%d?option=%s&option=%s&option=%s&option=%s'>마지막</a>]", linkUrl, pageCount,option[0],option[1],option[2],option[3])); // 마지막으로 가는 페이지 형성
	  }
